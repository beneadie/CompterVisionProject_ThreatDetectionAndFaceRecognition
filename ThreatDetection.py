...
def analyze_threat(
          ...
          raised_arms_TF, face_covering_TF, high_risk_items_count_dict
          ):
     danger_score = 0
...
while cap.isOpened():
     ...
     Identify is the most important factor.
               ...
               if face_image.size > 0:
                         ...
                         if embedding is not None:
                              try:
                                   ...
                                   last_15_faces.append(prediction_face)
                                   #print("Predicted label:", prediction_face)
                                   cv2.putText(annotated_frame, f"Person: {prediction_face[0]}", (x, y - 10),
     ...
     if prediction_face:
          if prediction_face[1] < 0.85 or prediction_face[0] not in true_face_classes_dict:
               if person_detected:
     ...
     results_from_analysis_tuple = analyze_threat(last_15_faces, high_speed_object_count, grip_fist_TF, raised_arms_TF, facecover_detected, important_object_count)
